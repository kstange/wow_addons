...
local GetNumFriends, GetNumOnlineFriends, GetFriendInfoByIndex = C_FriendList.GetNumFriends, C_FriendList.GetNumOnlineFriends, C_FriendList.GetFriendInfoByIndex
...
local BNGetNumFriends, BNGetFriendInfo, BNGetGameAccountInfo, BNGetInfo = _G.BNGetNumFriends, C_BattleNet.GetFriendAccountInfo, C_BattleNet.GetGameAccountInfoByID, BNGetInfo
local BNGetFriendIndex, BNGetNumFriendGameAccounts, BNGetFriendGameAccountInfo = _G.BNGetFriendIndex, C_BattleNet.GetFriendNumGameAccounts, C_BattleNet.GetFriendGameAccountInfo
...
function _G.TitanPanelSocialButton_GetButtonText(id)
	...
	if TitanGetVar(TITAN_SOCIAL_ID, "ShowFriends") then
		table.insert(comps, "|cffFFFFFF"..GetNumOnlineFriends().."|r")
	end
...
local function getRealIDGroupIndicator(bnetIDAccount, playerRealmName)
	if TitanGetVar(TITAN_SOCIAL_ID, "ShowGroupMembers") then
		...
		for i = 1, BNGetNumFriendGameAccounts(index) do
			local _, client, _, _, _, _, characterName, realmName = BNGetFriendGameAccountInfo(index, i)
	...
	if index then
		...
		if numGameAccounts > 1 then
			...
			for i = 1, numGameAccounts do
				local bnetIDGameAccount, client, _, _, _, _, _, _, _, realmID, faction = BNGetFriendGameAccountInfo(index, i)
			...
			local bnetIDGameAccount = BNGetFriendInfo(index).gameAccountInfo.gameAccountID
...
local function parseRealID(filterClients)
	...
	local friends, bnets = {}, {}
	for i=1, numOnline do
		friendInfo = BNGetFriendInfo(i)
		local bnetIDAccount, accountName, battleTag, isBattleTagPresence = friendInfo.bnetAccountID, friendInfo.accountName, friendInfo.battleTag, friendInfo.isBattleTagFriend
		local isAFK, isDND, broadcastText, noteText = friendInfo.isAFK, friendInfo.isDND, friendInfo.customMessage, friendInfo.note
		...
		for j=1, BNGetNumFriendGameAccounts(i) do
			local gameAccountInfo = BNGetFriendGameAccountInfo(i, j)
			...
			if client and seen[bnetIDGameAccount] == nil then
				local toon = {
					...
					realmID = gameAccountInfo.realmID,
				...
				if client == BNET_CLIENT_WOW then
					if toon.zone and toon.zone ~= "" then
						if toon.realmName and toon.realmName ~= "" and toon.realmName ~= playerRealmName then
							toon.location = toon.zone.." - "..toon.realmName
						else
							if toon.projectID == 2 then
								toon.location = toon.zone.." - WoW Classic"
							else
								toon.location = toon.zone
							end
						end
					else
						toon.location = toon.realmName
					end
				else
					toon.location = toon.gameText
				end
				seen[bnetIDGameAccount] = toon
				if toon.client == "App" or toon.client == "BSAp" then
...
function countRealID(filterClients) -- local at top of file
	...
	for i=1, numOnline do
		...
		for j=1, BNGetNumFriendGameAccounts(i) do
			local client = BNGetFriendGameAccountInfo(i, j).clientProgram
			if client then
				if client == "App" then
					isBnet = true
				else
...
local function addFriends(tooltip, collapseVar)
	local numTotal, numOnline = GetNumFriends(), GetNumOnlineFriends()
	...
	for i=1, numOnline do
		...
		--local connected, name, class, area, playerNote, guid, level, playerDND, playerAFK, isRAF = GetFriendInfoByIndex(i)
		...
		-- Status icon
		left = left..getStatusIcon(chatFlag)

		-- Name
		left = left..colorText(name, info.className).." "

		-- Status
		left = left..getStatusText(chatFlag).." "

		-- Notes
		local playerNote = info.notes
